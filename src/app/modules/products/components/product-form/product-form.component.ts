...
export class ProductFormComponent implements OnInit, OnDestroy {
  ...
  getAllCategories(): void {
    this.categoriesService
      ...
      .subscribe({
        next: (response) => {
          console.log('action:', this.productAction?.event?.action);
          console.log('editAction:', this.editProductAction);
          console.log('productDatas:', this.productAction?.productDatas);
          console.log('event id:', this.productAction?.event?.id);
          if (response.length > 0) {
            this.categoriesDatas = response;

            //edit form trazer os dados do produto selecionado
            if (
      ...
      .subscribe({
        next: (response) => {
          if (response.length > 0) {
            ...
            this.productsDatas && this.productsDtService.setProductsData(this.productsDatas);
          }
        },
      });
  }
