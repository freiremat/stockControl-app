      ...
      this.categoriesService.editCategoryName(requestEditCategory)
        ...
        .subscribe({
          next: () => {
            ...
            this.messageService.add({ severity: 'success', summary: 'Sucesso', detail: 'Categoria editada com sucesso!', life: 3000 });
          },
          error: (error) => {
            console.log(error)
            this.categoryForm.reset();
            this.messageService.add({ severity: 'error', summary: 'Erro', detail: 'Ocorreu um erro ao editar a categoria.', life: 3000 });
          }
        });
    }
  }
      ...
      this.categoriesService.createNewCategory(requestCreateCategory)
        ...
        .subscribe({
          next: (response) => {
            if (response) {
              this.categoryForm.reset();
              this.messageService.add({ severity: 'success', summary: 'Sucesso', detail: 'Categoria criada com sucesso!', life: 3000 });
            }
          },
          error: (error) => {
            console.log(error)
            this.categoryForm.reset();
            this.messageService.add({ severity: 'error', summary: 'Erro', detail: 'Ocorreu um erro ao criar a categoria.', life: 3000 });
          }
        });
    }
  }

  //open form with category name to edit
  setCategoryName(category_name: string): void {
    if (category_name) {
      this.categoryForm.setValue({ name: category_name });
    }
  }
